...
// Code ran through Beautifier - http://jsbeautifier.org/ 
...
var Marker = function(data, gmap) {
    ...
    this.infocontent('<div id="content" class="text-center infocontent">' +
    ...
    $.ajax({
        ...
        success: function(data) {
            ...
            for (var i = 0; i < linkcount; i++) {
                var link = links[i];
                self.infocontent(self.infocontent() + '<a class="wikilinks" target="_blank" href="https://en.wikipedia.org/wiki/' + link + '">' + link + '</a>...');
            }
...
var ViewModel = function() {
    ...
    // An error function for failure to load Google Map
    this.mapfailure = function() {
        console.log('Something wrong with map');
        document.getElementById('map').innerhtml = "<h1>NO MAP FOUND</h1>";
    };
    ...
    this.filterlocations = function() {
        ...
        for (var i = 0; i < self.placeList().length; i++) {
            if (self.placeList()[i].title().toLowerCase().search(self.locsearch().toLowerCase()) >= 0) {
                self.filterList.push({
                    name: self.placeList()[i].title()
                });
                // console.log(self.placeList()[i].marker.visible);
    ...
    this.changemarker = function(clickedloc) {
        for (var i = self.placeList().length - 1; i >= 0; i--) {
            if (self.placeList()[i].title() == clickedloc.name) {
                ...
                self.placeList()[i].location().lat += self.map.zoom * 0.00005;
                self.placeList()[i].infowindow.setOptions({
                    position: self.placeList()[i].location()
                });
                self.placeList()[i].infowindow.open(self.map);
                self.placeList()[i].marker.setAnimation(google.maps.Animation.BOUNCE);
                self.placeList()[i].location().lat -= self.map.zoom * 0.00005;
                // console.log(self.placeList()[i].location().lat);
